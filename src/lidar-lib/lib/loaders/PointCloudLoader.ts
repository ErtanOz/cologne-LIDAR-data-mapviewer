...
// Define common European projection (ETRS89 / UTM zone 32N) often used in Germany
...
export class PointCloudLoader {
  ...
  private async _loadWithLoadersGL(buffer: ArrayBuffer): Promise<PointCloudData> {
    ...
    if (!wkt) {
      console.warn('No WKT found, assuming EPSG:25832 (ETRS89 / UTM zone 32N) for local data');
      wkt = 'EPSG:25832';
    }
